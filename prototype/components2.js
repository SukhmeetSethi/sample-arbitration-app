...
function ArbitratorMarketplace({ arbitrators, role, onNavigate }) {
    ...
    <div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          {role === 'arbitrator' && <button className="btn btn-primary" onClick={() => setShowOnboard(true)}>+ Register as Arbitrator</button>}
        </div>
        <div className="card-grid">
          {filtered.map(a => (
            <div key={a.id} className="arb-card">
              ...
              <div className="arb-meta"><span>💰 {a.fee}</span></div>
              <div className="arb-meta">
                {a.domain.map(d => <span key={d} style={{background:'var(--bg)',padding:'2px 8px',borderRadius:4,fontSize:'0.75rem'}}>{d}</span>)}
              </div>
              <div style={{marginTop:10,display:'flex',gap:8}}>
                {role !== 'arbitrator' && <button className="btn btn-primary btn-sm" style={{flex:1}} disabled={!a.available}>{a.available ? 'Appoint' : 'Unavailable'}</button>}
                <button className="btn btn-outline btn-sm" style={{flex: role === 'arbitrator' ? 1 : 0}}>Profile</button>
              </div>
            </div>
          ))}
        </div>
    ...
    <Modal title="Arbitrator Registration" onClose={onClose} footer={
      ...
      {step === 2 && (
        <div>
          ...
          <div className="form-group"><label>Domain Expertise</label>
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {['Commercial','Construction','Employment','IP / Licensing','Insurance','Banking','Consumer','Property','International','Technology'].map(d => (
      ...
      {step === 3 && (
        <div>
          ...
          <p style={{marginTop:16,fontSize:'0.82rem',color:'var(--text-light)'}}>Your profile will be reviewed within 3–5 business days. Disclosure obligations under Section 12 (Fifth & Seventh Schedule) apply to all appointments.</p>
        </div>
      )}
    </Modal>
  );
  ...
  return (
    <div>
      <div className="card">
        ...
        <div className="table-wrap"><table><thead><tr><th>ID</th><th>Case</th><th>Date</th><th>Time</th><th>Type</th><th>Status</th><th>Action</th></tr></thead><tbody>
          {hearings.filter(h => h.status === 'Scheduled').map(h => (
            <tr key={h.id}><td>{h.id}</td><td style={{fontWeight:600}}>{h.caseId}</td><td>{h.date}</td><td>{h.time}</td><td>{h.type}</td><td><Badge status={h.status} /></td>
      ...
      <div className="card">
        ...
        <div className="table-wrap"><table><thead><tr><th>ID</th><th>Case</th><th>Date</th><th>Type</th><th>Duration</th><th>Status</th></tr></thead><tbody>
          {hearings.filter(h => h.status === 'Completed').map(h => (
            <tr key={h.id}><td>{h.id}</td><td style={{fontWeight:600}}>{h.caseId}</td><td>{h.date}</td><td>{h.type}</td><td>{h.duration}</td><td><Badge status={h.status} /></td></tr>
          ))}
        </tbody></table></div>
      </div>
    </div>
    ...
    <div>
      <div className="hearing-room">
        ...
        <div className="hearing-controls">
          <button className={`ctrl-btn ${muted ? 'off' : 'on'}`} onClick={() => setMuted(!muted)}>{muted ? '🔇' : '🎤'}</button>
          <button className={`ctrl-btn ${videoOff ? 'off' : 'on'}`} onClick={() => setVideoOff(!videoOff)}>{videoOff ? '📷' : '📹'}</button>
          <button className="ctrl-btn on">🖥️</button>
          <button className="ctrl-btn on">📄</button>
          <button className="ctrl-btn on">✋</button>
...
// ===== AWARDS PAGE =====
function AwardsPage({ role }) {
  return (
    <div>
      <div className="card">
        <div className="card-header"><h3>Awards</h3>{role === 'arbitrator' && <button className="btn btn-primary btn-sm">+ Draft New Award</button>}</div>
